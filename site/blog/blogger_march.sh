...
for f in March\ 2021/*.docx;
    ...
    echo $title " " $author;
    fname="${basefname// /}"
	$pandoc --lua-filter prepare_post.lua "$f" -o "$fname".md --extract-media="media-$fname" --wrap=none;
	$pandoc --lua-filter prepare_post.lua "$fname".md -o "$fname".html \
    	--data-dir="media-$fname" --metadata title="$title" --metadata author="$author";
    links="${links} * [$title](http://mpc.mit.edu/blog/$fname.html)\n";
done;
